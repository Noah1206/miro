    ...
    <AnimatePresence>
      {open && (
        <motion.div ref={ref} role="menu" tabIndex={-1} initial={{ opacity: 0, scale: 0.97, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: -4, transition: tween.exit }} transition={tween.enter}
          style={{ position: 'absolute', top: 'calc(100% + 6px)', [anchor]: 0, zIndex: 50, minWidth: 220, transformOrigin: `top ${anchor}`, background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: 6, boxShadow: 'var(--shadow-soft)' }}>
          {children}
        </motion.div>
      )}
    </AnimatePresence>
