  ...
  return (
    <div role="group" aria-label={label} style={{ display: 'inline-flex', padding: 3, gap: 2, background: 'var(--color-surface-1)', borderRadius: 'var(--radius-sm)' }}>
      {options.map((o) => {
        ...
        return (
          <button key={o.value} type="button" aria-pressed={on} onClick={() => { setLocal(o.value); onChange(o.value) }} style={{ position: 'relative', minHeight: size === 'sm' ? 28 : 36, padding: size === 'sm' ? '4px 10px' : '8px 14px', border: 0, background: 'transparent', borderRadius: 6, fontSize: size === 'sm' ? 'var(--font-micro)' : 'var(--font-caption)', letterSpacing: 0, textTransform: 'none', color: on ? 'var(--color-black)' : 'var(--color-text-secondary)', zIndex: 1 }}>
