 ...
 */
export function CharacterVisual({ name, accent, slug, photo, ratio = '4 / 5', shared = true, scrim = true, className, style }: {
  name: string; accent: string | null; slug: string
  /** 대표 사진 경로. 없으면 글자 자리표시. */
  photo?: string | null
  ratio?: string; shared?: boolean; scrim?: boolean; className?: string; style?: React.CSSProperties
}) {
  return (
