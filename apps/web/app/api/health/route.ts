...
import { currentMode, features } from '@miro/config'
...
export async function GET() {
  ...
  const body = {
    ...
    features: features(),
    providers: {
      llm: resolveLLM().info.mode, llmChain: resolveLLM().info.name, image: resolveImage().info.mode, push: resolvePush().info.mode,
