  ...
  return (
    <Page style={{ maxWidth: 520 }}>
      ...
      {u?.adultVerifiedAt ? <p data-verified className="t-body" style={{ padding: 16, background: 'var(--color-surface-1)', border: '1px solid var(--color-white)', borderRadius: 'var(--radius-md)' }}>인증 완료 · {u.adultVerifiedAt.toLocaleDateString('ko-KR')}</p>
        : locked ? <p data-verify-locked role="alert" className="t-body" style={{ padding: 16, background: 'var(--color-surface-1)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', color: 'var(--color-danger)' }}>인증에 실패했습니다. {verifyRetryAt(u!.adultVerifyFailedAt)!.toLocaleString('ko-KR')} 이후에 다시 시도할 수 있습니다.</p>
        : <VerifyForm />}
    </Page>
  )
}
