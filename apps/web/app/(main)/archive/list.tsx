 ...
 */
export function ArchiveList({ items, archiveAction, restoreAction }: {
  items: ArchiveItem[]
      ...
      <AnimatePresence initial={false}>
        {items.map((s) => {
          ...
          <motion.li key={s.id} data-session-row data-status={s.status} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -12, transition: tween.exit }} transition={spring.default}
            className="hoverable" style={{ background: 'var(--color-surface-1)', borderRadius: 'var(--radius-lg)', padding: 14 }}>
            <TransitionLink href={`/chat/${s.id}`} style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              <div aria-hidden style={{ width: 52, height: 64, borderRadius: 'var(--radius-sm)', flexShrink: 0, background: 'var(--color-surface-2)', display: 'grid', placeItems: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'baseline' }}>
                  <h2 className="t-title-3 t-name" style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    {s.characterName}
                    {/* 나뉜 탭 대신 행에서 상태를 밝힌다. */}
                    {archived && <span className="t-micro" style={{ color: 'var(--color-text-tertiary)' }}>보관됨</span>}
                  </h2>
