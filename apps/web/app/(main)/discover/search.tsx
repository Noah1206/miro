...
/**
 * 발견 머리 — 로고·제목 오른쪽의 돋보기를 누르면 검색창이 위에서 아래로 내려온다.
 * 검색어가 있으면 열린 채로 시작한다. Esc 나 X 로 닫고, 비우면 전체로 돌아간다.
 */
export function SearchHeader({ q, children }: { q: string; children: React.ReactNode }) {
  ...
  const submit = () => router.push(value.trim() ? `/discover?q=${encodeURIComponent(value.trim())}` : '/discover')
  const close = () => { setOpen(false); if (q) { setValue(''); router.push('/discover') } }

  return (
    <header style={{ padding: 'var(--space-5) var(--space-5) var(--space-4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        {children}
        <button type="button" onClick={() => (open ? close() : setOpen(true))} aria-expanded={open} aria-controls="discover-search" aria-label={open ? '검색 닫기' : '검색'}
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div id="discover-search" key="search"
            ...
            <form role="search" onSubmit={(e) => { e.preventDefault(); submit() }}
              ...
              {value && (
                <button type="button" onClick={() => { setValue(''); input.current?.focus(); if (q) router.push('/discover') }} aria-label="검색어 지우기"
