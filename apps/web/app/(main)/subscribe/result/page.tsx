...
export default async function Result({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const copy: Record<string, [string, string]> = {
    ...
    restored: ['구독이 돌아왔어요', '기존 구매의 자격이 이 계정에 연결되었습니다.'],
    nothing: ['복원할 구독이 없어요', '이 계정으로 확인되는 유효한 구매가 없습니다.'],
    failed: ['결제가 완료되지 않았어요', 'Pro 자격은 적용되지 않았습니다. 다시 시도하거나 구독 상태를 확인해 주세요.'],
  }
  ...
  return (
    <Page style={{ display: 'grid', placeItems: 'center', textAlign: 'center' }}>
      <Reveal>
        ...
        <TransitionLink href={ok ? '/my/subscription' : '/subscribe'} className="t-caption" style={{ textDecoration: 'underline', color: 'var(--color-text-primary)' }}>{ok ? '구독 상태 보기' : '다시 시도'}</TransitionLink>
      </Reveal>
    </Page>
  )
}
