...
export function MessageList({ items, characterName, portrait }: { items: Msg[]; characterName: string; portrait?: string | null }) {
  ...
  return (
    <div role="log" aria-live="polite" aria-relevant="additions" aria-label={characterName + ' 대화'} className={styles.messages}>
      <AnimatePresence initial={false}>
        {items.map((m) => <Line key={m.id}><Message m={m} characterName={characterName} portrait={portrait} /></Line>)}
      </AnimatePresence>
...
function Message({ m, characterName, portrait }: { m: Msg; characterName: string; portrait?: string | null }) {
    ...
    return (
      <Reportable id={m.id} kind="message">
        ...
        <div data-reality-message style={{ paddingLeft: 12, borderLeft: '2px solid var(--color-accent)' }}>
          <CharacterBubble m={m} name={characterName} portrait={portrait} />
        </div>
      </Reportable>
    )
  }
  return <Reportable id={m.id} kind="message"><CharacterBubble m={m} name={characterName} portrait={portrait} /></Reportable>
}

function CharacterBubble({ m, name, portrait }: { m: Msg; name: string; portrait?: string | null }) {
