...
/** 답장하기 — 세션을 만들고(이미 있으면 그대로) 대화로 들어간다. 회원가입 없음. */
export async function startExperience(): Promise<void> {
  ...
  if (!existing) trackAlpha(s.id, 'experience_start')
  redirect('/alpha/chat')
}
...
export async function joinWaitlist(_prev: WaitlistState, form: FormData): Promise<WaitlistState> {
  ...
  await db.insert(alphaWaitlist).values({ email, sessionId: session?.id ?? null }).onConflictDoNothing()
  trackAlpha(session?.id ?? null, 'waitlist_complete')
