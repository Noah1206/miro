    ...
    <main className="page page--immersive" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', position: 'relative', background: 'var(--color-bg-deep)' }}>
      ...
      <div style={{ position: 'relative', padding: '12px var(--space-4)', paddingBottom: 'calc(12px + env(safe-area-inset-bottom))', maxWidth: 720, width: '100%', margin: '0 auto' }}>
        ...
        <div role="group" aria-label="행동 제안" style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
          {suggestions.map((s) => <Button key={s} type="button" size="sm" variant="ghost" style={{ border: '1px solid var(--color-border)', background: 'rgba(10,10,11,0.5)' }} onClick={() => { if (ta.current) { ta.current.value = s; ta.current.focus() } }}>{s}</Button>)}
        </div>
