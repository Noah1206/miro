...
test('model menu defaults to MIRO and does not expose an unconfigured Pro model', async ({ page }) => {
  ...
  await picker.click()
  await expect(page.getByRole('dialog', { name: '모델과 요금제' })).toBeVisible()
  await expect(page.getByText(/하나의 월간 사용량/)).toBeVisible()
  await expect(page.getByRole('dialog').getByRole('button', { name: /부담 없이 시작하는 우리 이야기/ })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByRole('dialog').getByRole('button', { name: /MIRO Pro/ })).toBeDisabled()
