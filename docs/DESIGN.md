...
```css
:root {
  --color-accent: #E8590C;        /* 채움 */
  --color-accent-bright: #F26419; /* 눌림·호버에서 한 단 밝게 */
  --color-accent-on: #111111;     /* 주황 채움 위의 글자 */
  --color-accent-text: #E8590C;   /* 어두운 바탕 위 주황 글자 */
  --color-accent-soft: rgba(232, 89, 12, 0.06);
}
```

주황 위의 글자는 **반드시 검정**이다. 흰 글자는 3.6:1 이라 본문 기준(4.5:1)에 못 미친다
(검정은 5.3:1). `--color-accent-on` 이 그 값을 들고 있으니 버튼마다 다시 고르지 않는다.
